  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (err.code === 'auth/configuration-not-found') {
        ...
        setError(err.response?.data?.error || err.message || 'Admin sign in failed');
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex min-h-screen items-center justify-center px-4">
        <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl">
          ...
          <button
            ...
            disabled={loading}
            className="flex w-full items-center justify-center gap-3 rounded-lg bg-gray-900 px-4 py-3 font-medium text-white hover:bg-gray-800 disabled:opacity-50"
          >
